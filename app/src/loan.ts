...
async function main() {
  ...
  const loanIX = getFlashLoanInstruction({ amount: 7_000_000_000, borrower: payer.address, vault });
  ...
  const txMessage = pipe(
    ...
    (msg) => appendTransactionMessageInstructions([loanIX, logIX, repayIX], msg),
  );
